...
class Fireball(pygame.sprite.Sprite):
    def __init__(self,image, x, y, target_x,target_y):
        ...
        y_dist = -(target_y - y)
        self.angle = math.degrees(math.atan2(y_dist, x_dist))
        self.image = pygame.transform.rotate(self.original_image, self.angle - 90) # vì hình ảnh ban đầu đã quay 90 độ do Sprite
        self.rect = self.image.get_rect()
        self.rect.center = (x,y)
        # tính toán tốc độ theo trục x và y dựa vào góc bắn
    ...
    def update(self,screen_scroll,player):
        ...
        # kiểm tra bóng lửa ra khỏi màn hình hay chưa
        if self.rect.right < 0 or self.rect.left > constants.SCREEN_WIDTH or self.rect.bottom < 0 or self.rect.top > constants.SCREEN_HEIGHT:
            self.kill() 
        
        if self.rect.colliderect(player.rect) and player.hit == False:
            player.health -= 10
            player.hit = True
